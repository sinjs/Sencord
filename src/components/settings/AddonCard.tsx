...
export function AddonCard({ disabled, isNew, name, infoButton, footer, author, enabled, setEnabled, description, onMouseEnter, onMouseLeave, isSencord }: Props) {
    const titleRef = useRef<HTMLDivElement>(null);
    const titleContainerRef = useRef<HTMLDivElement>(null);

    return (
        <div
            className={cl("card", { "card-disabled": disabled, "card-sencord": isSencord })}
            ...
            <div className={cl("header")}>
                <div className={cl("name-author")}>
                    <Text variant="text-md/bold" className={cl("name")}>
                        ...
                        {isNew && <AddonBadge text="NEW" color="#ED4245" />}{isSencord && <AddonBadge text="SENCORD" color="#701a75" />}
                    </Text>
